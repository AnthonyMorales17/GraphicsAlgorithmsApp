...
namespace GraphicsAlgorithmsApp
{
    public partial class FrmBresenham : Form
    {
        ...
        private Pen drawPen = new Pen(Color.Red);
        private CBresenham bresenham;
        ...
        private List<CBresenham.StepData> lineSteps;
        ...
        private void btnGraph_Click(object sender, EventArgs e)
        {
            ...
            bresenham = new CBresenham(mCanvas, drawPen);
            lineSteps = bresenham.GetLineSteps(x1, y1, x2, y2, picCanvas.Width, picCanvas.Height);
            currentPixelIndex = 0;
            ...
            {
                lineTimer = new Timer();
                lineTimer.Interval = 50; // Ajusta el tiempo aquí
        ...
        {
            if (lineSteps == null || currentPixelIndex >= lineSteps.Count)
            {
                lineTimer.Stop();

                // Mostrar los datos en el DataGridView al finalizar
                dgvSteps.Rows.Clear();
                foreach (var step in lineSteps)
                    dgvSteps.Rows.Add(step.Pasos, step.X, step.Y, step.Pk, step.Decision, step.Accion);

                return;
            }

            var stepData = lineSteps[currentPixelIndex++];
            using (Graphics g = Graphics.FromImage(mCanvas))
            {
                g.FillRectangle(new SolidBrush(drawPen.Color), stepData.Pixel.X, stepData.Pixel.Y, CCoordinate.Scale, CCoordinate.Scale);
            }
            picCanvas.Refresh();
        }
        ...
        private void DrawAxes(Graphics g)
        {
            ...
            // Etiquetas eje X
            for (int i = 1; i <= 20 && centerX + i * scale < width; i++)
                g.DrawString(i.ToString(), font, textBrush, centerX + i * scale + 2, centerY + 2);
            for (int i = 1; i <= 20 && centerX - i * scale > 0; i++)
